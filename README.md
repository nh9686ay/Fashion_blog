...
1. Edit you `index.html` and `style.css` file and save your work
1. Notice that the two images for your reporduction are within this folder.
3. Make sure your two files (html + css) are within this directory
4. We'll walk through how to do a Pull Request on Tuesday so hand tight for submission instructions.
...
1. For your site navigation, you'll need to remove all of the padding on the `ul` elements and then set it's `list-style-type` to `none`. It would be a good idea to use a class to style the navigation so that you can still make a bulleted list on your page. Use Flexbox to style the `ul` for the navigation elements. On the top navigation use the `space-between` rule to have the menu items spread across their container evenly. Set the color for the anchors inside the main navigation to be `lightgray` and the footer navigation to be `tomato`. **We haven't learned Flexbox yet, so this requirement is optional**. Feel free to take a google for it, but this is quite a difficult skill to learn, and don't worry we'll cover it soon.
